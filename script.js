...
async function loadDropdown() {
  const response = await fetch("./puzzles/puzzleList.json?ts=" + Date.now());
...
function generateGrid(data) {
  ...
  if (!rows || !cols) {
    console.error("Invalid rows/cols:", data);
    return;
  }

  // Create empty grid safely
  const grid = [];
  ...
  data.words.forEach(wordObj => {
    ...
    for (let i = 0; i < word.length; i++) {
      ...
      if (r >= rows || c >= cols) {
        console.error("Word out of bounds:", wordObj);
        continue;
      }
